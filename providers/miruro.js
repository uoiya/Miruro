...
function getStreams(tmdbId, mediaType, seasonNum, episodeNum) {
  ...
  console.log('[Miruro] getStreams called with', tmdbId, mediaType, seasonNum, episodeNum);

  return getTmdbDetails(tmdbId, type)
    .then(function (meta) {
      console.log('[Miruro] TMDB resolved to:', meta.title, meta.year);
      ...
      return searchMiruro(query).then(function (results) {
        console.log('[Miruro] search "' + query + '" ->', results.length, 'results');
        var best = pickBestMatch(results, meta);
        if (!best) {
          if (query !== meta.title) {
            return searchMiruro(meta.title).then(function (fallbackResults) {
              console.log('[Miruro] fallback search "' + meta.title + '" ->', fallbackResults.length, 'results');
    ...
    .then(function (payload) {
      if (!payload || !payload.best) {
        console.log('[Miruro] no AniList match found, giving up');
        return [];
      }
      var anilistId = payload.best.id;
      var meta = payload.meta;
      console.log('[Miruro] matched AniList id', anilistId, 'for', meta.title);

      return getEpisodeProviders(anilistId).then(function (providersData) {
        var provList = Object.keys(providersData);
        console.log('[Miruro] episode providers available:', provList.join(', ') || '(none)');
    ...
    .then(function (finalStreams) {
      console.log('[Miruro] returning', (finalStreams || []).length, 'stream(s)');
      return finalStreams || [];
    })
    .catch(function (err) {
